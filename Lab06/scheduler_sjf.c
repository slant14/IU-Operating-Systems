...
static ProcessData find_next_process(void) {
    int location = -1;
    unsigned min_bt = UINT_MAX;

    for (int i = 0; i < (int)data_size; i++) {
        if (data[i].burst > 0 && (unsigned)data[i].at <= total_time && (unsigned)data[i].bt < min_bt) {
            min_bt = (unsigned)data[i].bt;
            location = i;
        }
    }
    ...
    while (location != -1 && (unsigned)data[location].at > total_time) {
        printf("Scheduler: Runtime: %u seconds.\nProcess %d: has not arrived yet.\n", total_time, location);
        total_time++;
    }

    if (location == -1) {
        ProcessData empty = {.idx = -1};
        return empty;
    }
